...
export default function Heart({ slug, compact = false }: { slug: string; compact?: boolean }) {
  ...
  const [piecesLeft, setPiecesLeft] = useState([...array15]);

  const allActive = activePieces.length === 15;

  useEffect(() => {
    const userId = getUserId();
    if (!userId) return;

    fetch(`/api/likes?slug=${slug}&userId=${userId}`)
      .then(res => res.json())
      .then(data => {
        if (data.articleLikes !== undefined) {
          setArticleLikesCnt(data.articleLikes);
          const userLikes = data.userLikes || 0;
          const piecesLeftCopy = [...array15];
          const taken = piecesLeftCopy.splice(0, userLikes);
          setPiecesLeft(piecesLeftCopy);
          setActivePieces(taken);
        }
      })
      .catch(() => {});
  }, [slug]);
  ...
  return (
    <div
      className={`outline-none cursor-pointer flex items-center text-lg text-text max-tablet:flex-row-reverse ${
        animate && allActive ? 'animate-heartbeat' : ''
      }`}
      onClick={activatePiece}
      onMouseDown={e => e.preventDefault()}
